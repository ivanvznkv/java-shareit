...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemResponseDto createItem(Long ownerId, ItemCreateRequest createRequest) {
        ...
        if (createRequest.getRequestId() != null) {
            itemRequest = itemRequestRepository.findById(createRequest.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запрос с id " + createRequest.getRequestId() + " не найден"));
        }

        Item item = ItemMapper.fromCreateRequest(createRequest, owner, itemRequest);
    ...
    public List<ItemWithBookingsDto> getItemsByOwner(Long ownerId) {
        ...
        Map<Long, BookingShortDto> lastBookingMap = new HashMap<>();
        Map<Long, BookingShortDto> nextBookingMap = new HashMap<>();
        ...
        for (Long itemId : itemIds) {
            ...
            if (last != null) {
                lastBookingMap.put(itemId, BookingMapper.toShortDto(last));
            }
            if (next != null) {
                nextBookingMap.put(itemId, BookingMapper.toShortDto(next));
            }
        }
